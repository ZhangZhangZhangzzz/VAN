...
float Q_angle = 0.08;		//角度数据置信度，角度噪声的协方差（过程噪声方差）
float Q_gyro  = 0.08;		//角速度数据置信度，角速度噪声的协方差（过程噪声方差）
float R_angle = 0.001	;	//加速度计测量噪声的协方差（观测噪声方差）
float dt      = 0.08;		//滤波算法计算周期，由定时器定时20ms
char  C_0     = 1;			//H矩阵值
